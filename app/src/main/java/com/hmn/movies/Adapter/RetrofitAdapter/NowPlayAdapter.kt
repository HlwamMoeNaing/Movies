...
class NowPlayAdapter(val ctx:Context, val list: List<Result2>):RecyclerView.Adapter<RecyclerView.ViewHolder> (){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as NowPlay).bind(list[position].title,list[position].release_date,list[position].poster_path)
